...
SELECT Name, Color
FROM Production.Product
WHERE (Name IN ('LL Crankarm', 'ML Crankarm', 'HL Crankarm', 'Chainring') AND Color = 'Black') OR (Name IN ('Chainring Bolts', 'Chainring Nut') AND Color = 'Silver')
...
--12. Write a query that retrieves the unique combination of columns ProductSubcategoryID and Color from the Production.Product table. We do not want any rows that are NULL.in any of the two columns in the result. 
--(Hint: Use IsNull)
SELECT DISTINCT ISNULL(ProductSubcategoryID, -1) AS ProductSubcategoryID, ISNULL(Color, 'N/A') AS Color
FROM Production.Product